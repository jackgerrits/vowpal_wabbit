...
base_learner* cb_dro_setup(options_i& options, workspace& all)
{
  ...
  if (options.was_supplied("no_predict")) { throw vw::error(vw::error_code::unknown, "cb_dro cannot be used with no_predict"); }

  if (!options.was_supplied("cb_adf") && !options.was_supplied("cb_explore_adf"))
  { throw vw::error(vw::error_code::unknown, "cb_dro requires cb_adf or cb_explore_adf"); }

  if (alpha <= 0 || alpha >= 1) { throw vw::error(vw::error_code::unknown, "cb_dro_alpha must be in (0, 1)"); }

  if (tau <= 0 || tau > 1) { throw vw::error(vw::error_code::unknown, "cb_dro_tau must be in (0, 1]"); }

  if (wmax <= 1) { throw vw::error(vw::error_code::unknown, "cb_dro_wmax must exceed 1"); }
  ...
  if (!data->isValid()) { throw vw::error(vw::error_code::unknown, "invalid cb_dro parameter values supplied"); }
