...
// iterate through one namespace (or its part), callback function FuncT(some_data_R, feature_value_x, feature_weight)
template <class DataT, class WeightOrIndexT, void (*FuncT)(DataT&, float, WeightOrIndexT)>
inline void foreach_feature(workspace& all, example& ec, DataT& dat, size_t& num_interacted_features)
{
...
inline void generate_interactions(workspace& all, example_predict& ec, R& dat, size_t& num_interacted_features)
{
