...
namespace LabelDict
{
void del_example_namespace(example& ec, VW::strong_namespace_index ns, features& fs)
{
  ...
  features& del_target = ec.feature_space[ns];
  assert(del_target.size() >= fs.size());
  assert(ec.indices.size() > 0);
  if (ec.indices.back() == ns && ec.feature_space[ns].size() == fs.size()) ec.indices.pop_back();
...
void add_example_namespace(example& ec, VW::strong_namespace_index ns, features& fs)
{
  bool has_ns = false;
  ...
  features& add_fs = ec.feature_space[ns];
...
void del_example_namespaces_from_example(example& target, example& source)
{
  if (source.indices.size() == 0)  // making sure we can deal with empty shared example
    return;
  VW::strong_namespace_index* idx = source.indices.end();
  idx--;
...
void add_example_namespace_from_memory(label_feature_map& lfm, example& ec, size_t lab)
{
  ...
  add_example_namespace(ec, VW::strong_namespace_index{'l'}, res_iter->second);
}
...
void del_example_namespace_from_memory(label_feature_map& lfm, example& ec, size_t lab)
{
  auto res_iter = lfm.find(lab);
  if (res_iter == lfm.end()) return;
  del_example_namespace(ec, VW::strong_namespace_index{'l'}, res_iter->second);
}
