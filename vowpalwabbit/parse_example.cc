...
public:
  ...
  TC_parser(vw::string_view line, workspace& all, example* ae) : _line(line)
  {
...
namespace vw
{
void read_line(workspace& all, example* ex, vw::string_view line)
{
  while (line.size() > 0 && line.back() == '\n') line.remove_suffix(1);
  substring_to_example(&all, ex, line);
}

void read_line(workspace& all, example* ex, char* line) { return read_line(all, ex, vw::string_view(line)); }
