...
int read_cached_features(workspace* all, v_array<example*>& examples)
{
  ...
  size_t total = all->example_parser->lbl_parser.read_cached_label(&ae->l, ae->_reduction_features, *input);
  if (total == 0) return 0;
...
{
  if (number > UINT32_MAX) { throw vw::error(vw::error_code::unknown, "size_t value is out of bounds of uint32_t.") }
