...
void accumulate(workspace& all, parameters& weights, size_t offset)
{
...
void accumulate_avg(workspace& all, parameters& weights, size_t offset)
{
...
template <class T>
void do_weighting(workspace& all, uint64_t length, float* local_weights, T& weights)
{
  for (uint64_t i = 0; i < length; i++)
  {
