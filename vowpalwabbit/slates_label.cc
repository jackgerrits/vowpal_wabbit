...
void parse_label(slates::label& ld, const std::vector<std::string_view>& words, reduction_features&)
{
  ...
  if (words.empty()) { throw vw::error("Slates labels may not be empty"); }
  if (!(words[0] == SLATES_LABEL)) { throw vw::error("Slates labels require the first word to be slates"); }

  if (words.size() == 1) { throw vw::error("Slates labels require a type. It must be one of: [shared, action, slot]"); }
  ...
  if (type == SHARED_TYPE)
  {
    ...
    {
      throw vw::error("Slates shared labels must be of the form: slates shared [global_cost]");
    }
    ld.type = example_type::shared;
  }
  else if (type == ACTION_TYPE)
  {
    if (words.size() != 3) { throw vw::error("Slates action labels must be of the form: slates action <slot_id>"); }
    ...
    { throw vw::error("Slot id seems to be malformed"); }
  ...
  else if (type == SLOT_TYPE)
  {
    if (words.size() == 3)
    {
      ...
      for (auto& token : tokenize(',', words[2]))
      {
        ...
        if (split_colons.size() != 2) { throw vw::error("Malformed action score token"); }
  ...
  {
    throw vw::error(fmt::format("Unknown slates label type: {}", type));
  }
}
