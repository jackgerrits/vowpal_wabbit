...
void predict_eval(cb&, single_learner&, example&) { throw vw::error("can not use a test label for evaluation"); }
...
base_learner* cb_algs_setup(options_i& options, workspace& all)
{
  ...
  {
    if (eval) throw vw::error("direct method can not be used for evaluation --- it is biased.");
