...
namespace detail
{
spdlog::logger* get_stderr_logger();
}
...
{
  detail::get_stderr_logger()->info(fmt, std::forward<Args>(args)...);
}

template <typename FormatString, typename... Args>
void errlog_warn(const FormatString& fmt, Args&&... args)
{
  detail::get_stderr_logger()->warn(fmt, std::forward<Args>(args)...);
}

template <typename FormatString, typename... Args>
void errlog_error(const FormatString& fmt, Args&&... args)
{
  detail::get_stderr_logger()->error(fmt, std::forward<Args>(args)...);
}

template <typename FormatString, typename... Args>
void errlog_critical(const FormatString& fmt, Args&&... args)
{
  detail::get_stderr_logger()->critical(fmt, std::forward<Args>(args)...);
}
