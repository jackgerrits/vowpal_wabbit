...
struct bfgs
{
  workspace* all = nullptr;  // prediction, regressor
  int m = 0;
...
void zero_derivative(workspace& all) { all.weights.set_zero(W_GT); }

void zero_preconditioner(workspace& all) { all.weights.set_zero(W_COND); }

void reset_state(workspace& all, bfgs& b, bool zero)
{
  b.lastj = b.origin = 0;
...
float bfgs_predict(workspace& all, example& ec)
{
...
float predict_and_gradient(workspace& all, example& ec)
{
...
void update_preconditioner(workspace& all, example& ec)
{
...
float dot_with_direction(workspace& all, example& ec)
{
...
template <class T>
double regularizer_direction_magnitude(workspace& /* all */, bfgs& b, double regularizer, T& weights)
{
  double ret = 0.;
...
double regularizer_direction_magnitude(workspace& all, bfgs& b, float regularizer)
{
...
template <class T>
float direction_magnitude(workspace& /* all */, T& weights)
{
  // compute direction magnitude
  double ret = 0.;
  for (typename T::iterator iter = weights.begin(); iter != weights.end(); ++iter)
...
float direction_magnitude(workspace& all)
{
  // compute direction magnitude
  if (all.weights.sparse)
...
template <class T>
void bfgs_iter_start(workspace& all, bfgs& b, float* mem, int& lastj, double importance_weight_sum, int& origin, T& weights)
{
  double g1_Hg1 = 0.;
  double g1_g1 = 0.;
...
void bfgs_iter_start(workspace& all, bfgs& b, float* mem, int& lastj, double importance_weight_sum, int& origin)
{
  if (all.weights.sparse)
...
template <class T>
void bfgs_iter_middle(workspace& all, bfgs& b, float* mem, double* rho, double* alpha, int& lastj, int& origin, T& weights)
{
  float* mem0 = mem;
...
void bfgs_iter_middle(workspace& all, bfgs& b, float* mem, double* rho, double* alpha, int& lastj, int& origin)
{
  if (all.weights.sparse)
...
template <class T>
double wolfe_eval(workspace& all, bfgs& b, float* mem, double loss_sum, double previous_loss_sum, double step_size,
...
template <class T>
double add_regularization(workspace& all, bfgs& b, float regularization, T& weights)
{
...
double add_regularization(workspace& all, bfgs& b, float regularization)
{
  if (all.weights.sparse)
...
template <class T>
void finalize_preconditioner(workspace& /* all */, bfgs& b, float regularization, T& weights)
{
...
void finalize_preconditioner(workspace& all, bfgs& b, float regularization)
{
  if (all.weights.sparse)
...
template <class T>
void preconditioner_to_regularizer(workspace& all, bfgs& b, float regularization, T& weights)
{
...
void preconditioner_to_regularizer(workspace& all, bfgs& b, float regularization)
{
  if (all.weights.sparse)
...
template <class T>
void regularizer_to_weight(workspace& /* all */, bfgs& b, T& weights)
{
  if (b.regularizers != nullptr)
  {
    for (typename T::iterator w = weights.begin(); w != weights.end(); ++w)
...
void regularizer_to_weight(workspace& all, bfgs& b)
{
  if (all.weights.sparse)
...
template <class T>
double derivative_in_direction(workspace& /* all */, bfgs& b, float* mem, int& origin, T& weights)
{
  double ret = 0.;
  for (typename T::iterator w = weights.begin(); w != weights.end(); ++w)
  {
...
double derivative_in_direction(workspace& all, bfgs& b, float* mem, int& origin)
{
  if (all.weights.sparse)
...
template <class T>
void update_weight(workspace& /* all */, float step_size, T& w)
{
  for (typename T::iterator iter = w.begin(); iter != w.end(); ++iter)
    (&(*iter))[W_XT] += step_size * (&(*iter))[W_DIR];
}

void update_weight(workspace& all, float step_size)
{
  if (all.weights.sparse)
...
int process_pass(workspace& all, bfgs& b)
{
  int status = LEARN_OK;
...
void process_example(workspace& all, bfgs& b, example& ec)
{
  label_data& ld = ec.l.simple;
...
void save_load_regularizer(workspace& all, bfgs& b, io_buf& model_file, bool read, bool text)
{
  int c = 0;
...
base_learner* bfgs_setup(options_i& options, workspace& all)
{
