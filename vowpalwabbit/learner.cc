...
namespace LEARNER
{
void learn_ex(example& ec, workspace& all)
{
  all.learn(ec);
  as_singleline(all.l)->finish_example(all, ec);
}

void learn_multi_ex(multi_ex& ec_seq, workspace& all)
{
  all.learn(ec_seq);
  as_multiline(all.l)->finish_example(all, ec_seq);
}

void end_pass(example& ec, workspace& all)
{
  all.current_pass++;
  all.l->end_pass();
...
void save(example& ec, workspace& all)
{
  // save state command
...
inline bool example_is_newline_not_header(example& ec, workspace& all)
{
...
void drain_examples(workspace& all)
{
...
{
public:
  single_instance_context(workspace& all) : _all(all) {}

  workspace& get_master() const { return _all; }

  template <class T, void (*process_impl)(T&, workspace&)>
  void process(T& ec)
  {
...
{
public:
  multi_instance_context(const std::vector<workspace*>& all) : _all(all) {}

  workspace& get_master() const { return *_all.front(); }
...
{
public:
  ready_examples_queue(workspace& master) : _master(master) {}
...
void generic_driver(const std::vector<workspace*>& all)
{
...
void generic_driver_onethread(workspace& all)
{
  single_instance_context context(all);
  handler_type handler(context);
  auto multi_ex_fptr = [&handler](workspace& all, v_array<example*> examples) {
