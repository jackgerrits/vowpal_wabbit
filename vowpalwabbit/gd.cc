...
struct gd
{
  ...
  bool adax;
  workspace* all;  // parallel, features, parameters
};

void sync_weights(workspace& all);
...
struct audit_results
{
  ...
  audit_results(workspace& p_all, const size_t p_offset) : all(p_all), offset(p_offset) {}
};
...
inline float trunc_predict(workspace& all, example& ec, double gravity)
{
...
template <class T>
void save_load_regressor(workspace& all, io_buf& model_file, bool read, bool text, T& weights)
{
  size_t brw = 1;
...
void save_load_online_state(
    workspace& all, io_buf& model_file, bool read, bool text, double& total_weight, gd* g, uint32_t ftrl_size)
{
  std::stringstream msg;
...
uint64_t set_learn(workspace& all, gd& g)
{
...
template <bool sparse_l2, bool invariant, bool sqrt_rate, uint64_t adaptive, uint64_t normalized, uint64_t spare,
    uint64_t next>
uint64_t set_learn(workspace& all, bool feature_mask_off, gd& g)
{
  all.normalized_idx = normalized;
  if (feature_mask_off)
