...
vw::LEARNER::base_learner* setup(vw::config::options_i& options, workspace& all)
{
  ...
  if (synthcoversize <= 0) { throw vw::error(vw::error_code::unknown, "synthcoversize must be >= 1"); }
  if (epsilon < 0) { throw vw::error(vw::error_code::unknown, "epsilon must be non-negative"); }
  if (psi <= 0) { throw vw::error(vw::error_code::unknown, "synthcoverpsi must be positive"); }
