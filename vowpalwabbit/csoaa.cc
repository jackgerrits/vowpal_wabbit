...
bool ec_seq_is_label_definition(multi_ex& ec_seq)
{
  ...
  for (size_t i = 1; i < ec_seq.size(); i++)
    if (is_lab != ec_is_label_definition(*ec_seq[i])) throw vw::error(vw::error_code::unknown, "error: mixed label definition and examples in ldf data!");
  return is_lab;
}
...
multi_ex process_labels(ldf& data, const multi_ex& ec_seq_all)
{
  ...
  if (ec_seq_has_label_definition(ec_seq_all)) { throw vw::error(vw::error_code::unknown, "error: label definition encountered in data block"); }
...
base_learner* csldf_setup(options_i& options, workspace& all)
{
  ...
  {
    if (all.training) throw vw::error(vw::error_code::unknown, "ldf requires either m/multiline or mc/multiline-classifier");
