...
{
  if (all.sd->max_label < all.sd->min_label) { throw vw::error("Max label cannot be less than min label."); }
}
...
void validate_default_bits(workspace& all, uint32_t local_num_bits)
{
  ...
  {
    throw vw::error(
        fmt::format("-b bits mismatch: command-line {} != {} stored in model", all.num_bits, local_num_bits));
  }
}
