...
{
  if (ec_seq.empty()) throw vw::error("cb_adf: At least one action must be provided for an example to be valid.");
  ...
  for (auto* ec : ec_seq)
  {
    ...
    if (ec->l.cb.costs.size() > 1) throw vw::error("cb_adf: badly formatted example, only one cost can be known.");
    ...
    if (ec->l.cb.costs.size() == 1 && ec->l.cb.costs[0].cost != FLT_MAX)
    {
      ret = ec;
      count += 1;
      if (count > 1) throw vw::error("cb_adf: badly formatted example, only one line can have a cost");
    }
  }
  ...
  if (test_adf_sequence(ec_seq) != nullptr)
  {
    ...
    switch (_gen_cs.cb_type)
    {
      ...
      case CB_TYPE_SM:
        ...
        break;
      default:
        throw vw::error(fmt::format("Unknown cb_type specified for contextual bandit learning: {}", _gen_cs.cb_type));
    }
  }
