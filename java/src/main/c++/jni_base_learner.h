...
example* read_example(JNIEnv* env, jstring example_string, workspace* vwInstance);
example* read_example(const char* example_string, workspace* vwInstance);
...
T base_predict(JNIEnv* env, example* ex, bool learn, workspace* vwInstance, const F& predictor, const bool predict)
{
  T result = 0;
  try
  {
