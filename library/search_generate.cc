...
public:
  ...
  Generator(workspace& vw_obj, Trie* _dict=nullptr) : SearchTask<input,output>(vw_obj), dist(0), dict(_dict)    // must run parent constructor!
  {
...
void run_easy()
{ workspace& vw_obj = *vw::initialize("--search 29 --quiet --search_task hook --ring_size 1024 --search_rollin learn --search_rollout none");
...
{ // initialize VW as usual, but use 'hook' as the search_task
  ...
  workspace& vw_obj = *vw::initialize(init_str);
  cerr << init_str << endl;
