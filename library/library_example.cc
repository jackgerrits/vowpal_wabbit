...
inline feature vw_feature_from_string(workspace& v, std::string fstr, unsigned long seed, float val)
...
{ workspace* model = vw::initialize("--hash all -q st --noconstant -i train.w -f train2.vw --no_stdin");
  ...
  workspace* model2 = vw::initialize("--hash all -q st --noconstant -i train2.vw --no_stdin");
